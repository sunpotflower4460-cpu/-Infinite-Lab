...
- **ふつう / じっくり**：ふだんは「ふつう」の AI（OpenAI・Qwen・MiniMax・DeepSeek から選ぶ。既定のモデル名は `gpt-6-luna`・`MiniMax-M3`・`deepseek-flash`＝V4.1 Flash、いずれも画像を読める。2026-09 時点）が答え、答えの下の「じっくり聞き直す」を押したときだけ「じっくり」の AI（既定は Claude `claude-opus-5`）に聞く。2 つの答えは並べて残る
- ⚙ 設定：AI ごとに API キー・モデル名・接続先 URL・画像を送るかを設定。キーは**このブラウザにだけ**保存され、その AI の接続先にだけ送られる（書き出し・履歴には入らない）。ブラウザから直接つながらない（CORS）ときは、接続先を中継サーバーに変えられる
- Claude は公式 SDK（`@anthropic-ai/sdk`、ブラウザから利用者のキーで直接）で呼ぶ。Opus 5 / Fable 5 ではサーバー側フォールバック（`fallbacks: "default"`）を有効にしている
