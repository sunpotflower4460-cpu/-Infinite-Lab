...
上部の **φ と π の部屋** ボタン（または URL の末尾に `#golden`）で開く解説ページです。黄金比 φ と π の関係を、その場で計算した図と数値で見せます。

| 項目                   | 見せていること                                                                                                                                                   |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 正五角形             | 対角線 ÷ 辺 = 2·cos(π/5) = φ（図の頂点から測定）                                                                                                                 |
| 2 黄金角               | ひまわりの種の並び（Vogel のモデル）。角度が一周の分数に近いと腕が出て、黄金角 360°/φ² ≈ 137.508° では隙間なく詰まる                                             |
| 3 分数への近さ         | π と φ の連分数と近似分数（BigInt で全桁を保証した値から）。q² × ずれ：π は 355/113 で 0.003、φ はずっと 1/√5 ≈ 0.447                                            |
| 4 トーラスを埋める速さ | 切り開いたトーラス上の線 y = a·t（mod 1）が埋めるマスの割合（測定）。φ² は π より速く、π は 113 周で埋まり切り、22/7 は 7 周で閉じる                             |
| 5 KAM                  | 標準写像。回転の比 π−3・e−2・√2−1・1/φ² の不変曲線（トーラス）が残るかを K ごとに判定（有限回の計算）。黄金比が最後まで残る（文献値 K ≈ 0.9716）。「壁」テストも |
| 6 偶然の一致           | π ≈ 4/√φ は 0.096% ずれる偶然の一致（355/113 のずれの 1 万倍以上）                                                                                               |

計算は `src/golden/golden.ts`（sin / cos は決定的な実装）、テストは `tests/unit/golden/`。

## AI ガイド（ペンで囲んで聞く）

どの画面でも、右下の **✎ 囲んで聞く** を押して気になるところをペンで丸く囲むと、その部分について AI に質問できます。

- AI に渡すもの：囲んだ部分の**画像**（画像を読めるモデルのとき）、囲んだ範囲にある**文字**、**場所**（例「φ と π の部屋 › 4 トーラスを埋める速さ」）、その画面が報告する**状態**（ラボなら実験・式・パラメータ・step と、その step の計算値、部屋ならスライダーの値や判定結果）。パネルの「AI に渡す内容を見る」でそのまま確認できる
- **3 つのクラス**：質問にはまず **下** が答え、足りないときは答えの下のボタンで **中**・**上** に聞き直す（答えは並べて残る）。下のモデルはパネルですぐ切り替えられる。答えごとに、AI が報告したトークン数と定価からの料金の目安を表示

  | クラス | モデル（既定は太字）                                            | 1 回の目安 |
  | ------ | --------------------------------------------------------------- | ---------- |
  | 下     | **GPT-6 Luna**・DeepSeek V4.1 Flash・Qwen 3.8 Flash・MiniMax M3 | 0.1 円前後 |
  | 中     | **GPT-6 Sol**・Qwen 3.8 Max・Claude Sonnet 5                    | 1〜2 円    |
  | 上     | **Claude Opus 5.5**                                             | 3 円前後   |

  値段・点数（Artificial Analysis v4.3.2）は 2026 年 9 月時点（`src/guide/providers.ts` の `MODELS`）。モデル名は設定で直せる

- ⚙ 設定：API キーと接続先は会社ごと（例：Luna と Sol は同じ OpenAI のキー）、モデル名と画像を送るかはモデルごと。キーは**このブラウザにだけ**保存され、その AI の接続先にだけ送られる（書き出し・履歴には入らない）。ブラウザから直接つながらない（CORS）ときは、接続先を中継サーバーに変えられる
- Claude は公式 SDK（`@anthropic-ai/sdk`、ブラウザから利用者のキーで直接）で呼ぶ。Opus 5 系（Opus 5.5 を含む）と Fable 5 系ではサーバー側フォールバック（`fallbacks: "default"`）を有効にしている
- AI の答えは推測を含む。アプリが計算した値と見比べる前提の表示にしている

**新しい画面・部屋を AI ガイドに対応させる**（`src/guide/context.ts`）：

- 画面の状態：`useGuideContext('my-room', () => ({ page: '…', … }))`
- 項目の名前と要約：`data-guide-title="…"`（場所として送られる）、`data-guide-summary`（その項目の要約）
- 囲みの外でも送りたい値（スライダーなど）：`data-guide-state`
- WebGL の canvas で、描画内容を保持しないもの：`registerCanvasSnapshot(canvas, () => 画像)`（`src/guide/capture.ts`）
