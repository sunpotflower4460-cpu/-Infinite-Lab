...
アプリを開くと、まずこの画面から始まります。✕ で実験画面へ（URL は `#lab` になり、再読み込みしても実験画面のまま）。実験画面からは上部の **▶ π の模様を見る** で戻れます。実験画面を直接開くには URL の末尾に `#lab` を付けます（例: `http://localhost:5173/#lab`）。

説明は 3 段階で切り替えられます（選んだものはブラウザに記憶）:

| 表示         | 内容                                                                                                                                                                       |
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **かんたん** | `π = 3.14159…`、根元の腕と先の腕の回転数とその比（= 3.14159）、いま起きていること（7 周で π ≈ 22/7 に近づき 15 枚の花びら → 無理数なので戻らず網目 → 355/113 …）。腕も表示 |
| **専門**     | 実行中の式と値（θ₁ = (n × dt) mod 2π、θ₂ = (n × dt × π) mod 2π、x, y）、π の 40 桁、連分数の近似分数 p/q と p − q 回対称の表、計算方法（BigInt・誤差保証・決定性）         |
| **説明なし** | 参照動画と同じく線だけ                                                                                                                                                     |
...
| **06 Two-Arm 3D**         | 04 と同じ厳密な θ₁, θ₂ を **球**（θ₁ 経度・θ₂ 緯度）、**トーラス**（ρ = R + r·cos θ₂）、**高さ**（z ∝ t）の 3D の点に置く。右上の 2D / Sphere / Torus / Height で切り替え |
...
- **3D ビュー（v0.6）**: Two-Arm の角度を球・トーラス・高さで立体に。ドラッグで回転、クリックでその step の式を表示。three.js は 3D を開いたときだけ読み込む（詳細は [docs/EXPERIMENTS.md](docs/EXPERIMENTS.md)）
